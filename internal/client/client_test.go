...
func writeInstanceFiles(t *testing.T, files map[string]Instance) string {
	...
	dir := filepath.Join(home, ".unity-agent-cli", "instances")
...
// stopped instances, so `unity-agent-cli status` can display them.
...
func TestScanInstances_RemovesDeadPID(t *testing.T) {
	...
	deadPath := filepath.Join(home, ".unity-agent-cli", "instances", "dead.json")
...
func TestScanInstances_KeepsOnPermissionError(t *testing.T) {
	...
	fp := filepath.Join(home, ".unity-agent-cli", "instances", "eperm.json")
...
// TestFindActiveByPort_SkipsZeroTimestamp verifies that instances with
// Timestamp 0 are treated as incomplete and skipped.
